...
(function (window, _undefined) {
    ...
    function SoundManager(smURL, smID) {
        ...
        this.features = {
            "buffering": false, "peakData": false, "waveformData": false, "eqData": false, "movieStar": false
        };
        this.sandbox = {
            "type": null, "types": {
        ...
        var SMSound, sm2 = this, globalHTML5Audio = null, flash = null, sm = "soundManager", smc = sm + ": ",
            ...
            appendSuccess = false, didInit = false, disabled = false, windowLoaded = false, _wDS, wdCount = 0,
            initComplete, mixin, assign, extraOptions, addOnEvent, processOnEvents, initUserOnload, delayWaitForEI,
            waitForEI, rebootIntoHTML5, setVersionInfo, handleFocus, strings, initMovie, domContentLoaded, winOnLoad,
            didDCLoaded, getDocument, createMovie, catchError, setPolling, initDebug,
            ...
            toggleDebug, loopFix, policyFix, complain, idCheck, waitingForEI = false, initPending = false, startTimer,
            stopTimer, timerExecute, h5TimerCount = 0, h5IntervalTimer = null, parseURL, messages = [], canIgnoreFlash,
            needsFlash = null, featureCheck, html5OK, html5CanPlay, html5Ext, html5Unload, domContentLoadedIE,
            testHTML5, event, slice = Array.prototype.slice, useGlobalHTML5Audio = false, lastGlobalHTML5URL, hasFlash,
            detectFlash, badSafariFix, html5_events, showSupport, flushMessages, wrapCallback, idCounter = 0, didSetup,
            ...
            okToDisable = !tryInitOnFocus, flashMIME = /(mp3|mp4|mpa|m4a|m4b)/i, emptyURL = "about:blank",
        ...
        this.createSound = function (oOptions, _url) {
            ...
            if (options.id.toString().charAt(0).match(/^\d$/)) sm2._wD(cs + str("badID", options.id), 2);
        ...
        SMSound = function (oOptions) {
            ...
            this.load = function (oOptions) {
                ...
                if (html5OK(instanceOptions)) {
                    ...
                    try {
                        ...
                        instanceOptions = s._iO;
                        if (fV === 8) flash._load(s.id, instanceOptions.url, instanceOptions.stream, instanceOptions.autoPlay, instanceOptions.usePolicyFile); else flash._load(s.id, instanceOptions.url, !!instanceOptions.stream, !!instanceOptions.autoPlay, instanceOptions.loops || 1, !!instanceOptions.autoLoad, instanceOptions.usePolicyFile)
                    } catch (e) {
                        _wDS("smError", 2);
            ...
            detachOnPosition = function () {
                var item, op = s._iO.onposition;
                if (op) for (item in op) if (op.hasOwnProperty(item)) s.clearOnPosition(parseInt(item, 10))
            };
            ...
            this._whileplaying = function (nPosition, oPeakData, oWaveformDataLeft, oWaveformDataRight, oEQData) {
                ...
                if (!s.isHTML5 && fV > 8) {
                    if (instanceOptions.usePeakData && oPeakData !== _undefined && oPeakData) s.peakData = {
                        left: oPeakData.leftPeak, right: oPeakData.rightPeak
                    };
                    if (instanceOptions.useWaveformData && oWaveformDataLeft !== _undefined && oWaveformDataLeft) s.waveformData = {
                        left: oWaveformDataLeft.split(","), right: oWaveformDataRight.split(",")
                    };
        ...
        assign = function (o, oParent) {
            ...
            for (i in o) if (o.hasOwnProperty(i)) if (typeof o[i] !== "object" || o[i] === null || o[i] instanceof Array || o[i] instanceof RegExp) if (hasParent && bonusOptions[oParent] !== _undefined) sm2[oParent][i] = o[i]; else if (setupOptions[i] !== _undefined) {
                ...
                result = false
            } else if (sm2[i] instanceof Function) sm2[i].apply(sm2, o[i] instanceof Array ? o[i] : [o[i]]); else sm2[i] = o[i]; else if (bonusOptions[i] === _undefined) {
                complain(str(sm2[i] === _undefined ? "setupUndef" : "setupError", i), 2);
                result = false
        ...
        html5_events = {
            abort: html5_event(function () {
                ...
                if (ranges && ranges.length) {
                    for (i = 0, j = ranges.length; i < j; i++) s.buffered.push({
                        "start": ranges.start(i) * msecScale, "end": ranges.end(i) * msecScale
                    });
        ...
        testHTML5 = function () {
            ...
            for (item in aF) if (aF.hasOwnProperty(item)) {
                ...
                if (aF[item] && aF[item].related) for (i = aF[item].related.length - 1; i >= 0; i--) {
        ...
        disableObject = function (o) {
            var oProp;
            for (oProp in o) if (o.hasOwnProperty(oProp) && typeof o[oProp] === "function") o[oProp] = doNothing;
            oProp = null
        };
        ...
        featureCheck = function () {
            ...
            if (sm2.useHTML5Audio && sm2.hasHTML5) {
                ...
                for (item in formats) if (formats.hasOwnProperty(item)) if (formats[item].required) if (!sm2.html5.canPlayType(formats[item].type)) {
                    ...
                    flashNeeded = true
                } else if (sm2.preferFlash && (sm2.flash[item] || sm2.flash[formats[item].type])) flashNeeded = true
            }
            if (sm2.ignoreFlash) {
                flashNeeded = false;
                canIgnoreFlash = true
            ...
            if (oTarget) {
                ...
                if (!sm2.oMC.id) {
                    ...
                    if (!sm2.useFlashBlock) if (sm2.useHighPerformance) s = {
                        ...
                        s = {
                            "position": "absolute", "width": "6px", "height": "6px", "top": "-9999px", "left": "-9999px"
                        };
        ...
        waitForEI = function () {
            ...
            setTimeout(function () {
                ...
                } else if (sm2.flashLoadTimeout === 0) _wDS("waitForever"); else if (!sm2.useFlashBlock && canIgnoreFlash) rebootIntoHTML5(); else failSafely(true)
...
!function (e) {
    ...
    return function e(t, n, r) {
        ...
        1: [function (e, t, n) {
            ...
            t.exports = {
                name: "localStorage", read: o, write: i, each: u, remove: a, clearAll: c
            }
...
(function (a) {
    ...
    jQuery.fn.zvPlayer = function (l) {
        function k() {
            return d.find(".player-time-total").width()
        }
        ...
        function p() {
            a(".song-download").each(function (b, c) {
                var g = a(c).attr("data-url");
                "#" == g && (g = a(c).attr("data-href"));
                a(c).unbind("click");
                a(c).click(function () {
                    return a.zvPlayer.download(a(c), g)
                })
            })
        }

        function z() {
            a(".radio-play").each(function (b, c) {
                a(c).unbind("click");
                a(c).click(function () {
        ...
        function D() {
            v.volume && (a.zvPlayer.volume(w / 100), d.find(".player-volume-slider").unbind("click"), d.find(".player-volume-slider").click(function (b) {
                b = (b.pageX - a(this).offset().left) / 100;
                a.zvPlayer.volume(b, 1);
                0 != w ? d.find(".player-volume-button").removeClass("off") : d.find(".player-volume-button").addClass("off")
            }), d.find(".player-volume-slider").unbind("drag"), d.find(".player-volume-slider").bind("drag", function (b) {
                1 == b.which && (b = (b.pageX - a(this).offset().left) / 100, a.zvPlayer.volume(b), 0 != w ? d.find(".player-volume-button").removeClass("off") : d.find(".player-volume-button").addClass("off"))
            }), d.find(".player-volume-button").unbind("click"), d.find(".player-volume-button").click(function () {
                0 != w ? (a.zvPlayer.volume(0, 1), a(this).addClass("off")) : (a.zvPlayer.volume(a.cookie("ZvcurrentVolume") / 100, 1), a(this).removeClass("off"))
            }));
            d.find(".player-time-total, .player-time-loaded, .player-time-current, .player-song-name, .player-time-start, .player-time-end").unbind("click");
            d.find(".player-time-total, .player-time-loaded, .player-time-current, .player-song-name, .player-time-start, .player-time-end").click(function (b) {
                b = (b.pageX - a(this).parent().offset().left) / k();
                0 < e[f].duration ? h.setPosition(b * e[f].duration * 1E3) : h.setPosition(b * h.duration);
                y(b)
            });
            d.find(".player-nav-prev").unbind("click");
            d.find(".player-nav-prev").click(function () {
                a.zvPlayer.previous();
                return !1
            });
            d.find(".player-nav-play").unbind("click");
            d.find(".player-nav-play").click(function () {
                a.zvPlayer.toggle();
                return !1
            });
            d.find(".player-nav-next").unbind("click");
            d.find(".player-nav-next").click(function () {
                a.zvPlayer.next();
                return !1
            });
            d.find(".player-menu-plus").unbind("click");
            d.find(".player-menu-plus").click(function () {
                showTopNotify("<strong>\u0414\u043e\u0431\u0430\u0432\u044c\u0442\u0435 \u043a\u043e\u043c\u043f\u043e\u0437\u0438\u0446\u0438\u044e \u0432 \u043f\u043b\u0435\u0435\u0440</strong>");
                return !1
            });
            d.find(".player-menu-download").unbind("click");
            d.find(".player-menu-download").click(function () {
                showTopNotify("<strong>\u0414\u043e\u0431\u0430\u0432\u044c\u0442\u0435 \u043a\u043e\u043c\u043f\u043e\u0437\u0438\u0446\u0438\u044e \u0432 \u043f\u043b\u0435\u0435\u0440</strong>");
                return !1
            });
            A = !0;
            d.unbind("song-play");
            d.bind("song-play", function (a, c) {
                E && u(c, E)
            })
        }
        ...
        function H() {
            function b() {
                if (!A) return !1;
                var b = a(this);
                if ("true" == b.attr("pl")) e = [], r.find("#pl-song-list").children("div").each(function (b, c) {
                    ...
                    f = -1;
                    a.zvPlayer.addTrack(c, b.attr("data-title"), b.attr("data-sid"), !0, b.attr("data-time"))
                }
                return !1
            }

            var c = a("body");
            "1.7" <= d.jquery ? (c.off("click", ".song-play"), c.on("click", ".song-play", b)) : (c.undelegate(".song-play", "click"), c.delegate(".song-play", "click", b));
            ...
            -1 < f && (a("[data-sid]").removeClass("stop"), a("div[data-play]").removeClass("played"), a("[data-sid=" + e[f].id + "].song-play").addClass("stop"), a("div[data-play=" + e[f].id + "].").addClass("played"));
            c = a(".song-plus");
            c.unbind("click");
            c.click(function () {
        ...
        function K(b) {
            d.unbind("song-play");
            d.bind("song-play", function () {
                ++B;
                if (B < b.length) {
                    var c = b.eq(B).find(".song-play"), g = c.attr("data-url");
                    "#" == g && (g = c.attr("data-href"));
                    a.zvPlayer.addTrack(g, c.attr("data-title"), c.attr("data-sid"), !1, c.attr("data-time"))
                } else d.unbind("song-play"), D()
            })
        }

        function t(a) {
            for (var b = e.length, d = 0; d < e.length; ++d) if (a.title == e[d].title) return b = d;
        ...
        function u(b, c) {
            ...
            n = !c;
            d.find(".player-time-total, .player-time-loaded, .player-time-current, .player-song-name, .player-time-start, .player-time-end").css("cursor", "pointer");
            d.find(".player-time-total, .player-time-loaded, .player-time-current, .player-song-name, .player-time-start, .player-time-end").unbind("click");
            d.find(".player-time-total, .player-time-loaded, .player-time-current, .player-song-name, .player-time-start, .player-time-end").click(function (b) {
                b = (b.pageX - a(this).parent().offset().left) / k();
                0 < e[f].duration ? h.setPosition(b * e[f].duration * 1E3) : h.setPosition(b * h.duration);
            ...
            d.find(".player-time-loaded, .player-time-current").width(0);
            d.find(".player-time-start, .player-time-end").text("");
            d.find(".player-song-name").text(e[f].title);
            d.find(".player-menu-plus").attr("data-title", e[f].title);
            d.find(".player-menu-plus").attr("data-sid", e[f].id);
            d.find(".player-menu-plus").attr("data-url", e[f].stream_url);
            d.find(".player-menu-plus").attr("data-time", e[f].duration);
            d.find(".player-menu-plus").removeClass("in_playlist");
            d.find(".player-nav-prev").unbind("click");
            d.find(".player-nav-prev").click(function () {
                a.zvPlayer.previous();
                return !1
            ...
            d.find(".player-menu-plus").click(function () {
                if ("1" == isGuest) return window.location = "/login", !1;
                var b = a.zvPlayer.addToPlaylist(e[f].stream_url, e[f].title, e[f].id, e[f].duration), c = a(this),
                    d = "";
                !1 === b ? c.hasClass("in_playlist") || (c.addClass("in_playlist").attr("title", "\u0412 \u043f\u043b\u0435\u0439\u043b\u0438\u0441\u0442\u0435"), d = a('<div class="song_mess2"><div class="bg"><div class="cnt">\u041f\u0435\u0441\u043d\u044f \u0434\u043e\u0431\u0430\u0432\u043b\u0435\u043d\u0430<br/>\u0432 \u043f\u043b\u0435\u0439\u043b\u0438\u0441\u0442</div></div></div>')) : !0 !== b || c.hasClass("in_playlist") || (c.addClass("in_playlist").attr("title", "\u0412 \u043f\u043b\u0435\u0439\u043b\u0438\u0441\u0442\u0435"), d = a('<div class="song_mess2"><div class="bg"><div class="cnt">\u042d\u0442\u0430 \u043f\u0435\u0441\u043d\u044f \u0443\u0436\u0435<br/>\u0434\u043e\u0431\u0430\u0432\u043b\u0435\u043d\u0430 \u0432 \u043f\u043b\u0435\u0439\u043b\u0438\u0441\u0442</div></div></div>'));
                if ("" != d) {
                    d.fadeIn(500);
                    d.appendTo("body");
                    b = c.offset().left - d.outerWidth() / 2 + c.outerWidth() / 2;
                    var g = "center bottom";
                    c.hasClass("left_song_mess") && (b = c.offset().left - d.outerWidth() / 1.1 + c.outerWidth() / 2, g = "95% bottom");
                    c = c.offset().top - d.outerHeight() - 13;
                    d.css({top: c, left: b, display: "block", backgroundPosition: g});
                    setTimeout(function () {
            ...
            if (b = a("#SongView")) b.find(".left span").attr("class", "stop"), b.find(".cnt").text("\u041f\u0430\u0443\u0437\u0430");
            ...
            h = window.soundManager.createSound({
                id: "zv_sound" + e[f].id,
                url: e[f].stream_url + "?play=on&ww=" + b + "&wh=" + c,
                autoPlay: !0,
                autoLoad: !0,
                volume: w,
                multiShot: !1,
        ...
        function G(b, c) {
            ...
            a.zvPlayer.clear();
            d.find(".player-time-loaded, .player-time-current").width(0);
            d.find(".player-time-start").text("");
            d.find(".player-time-end").text("");
            d.find(".player-song-name").text(b);
            d.find(".player-menu-plus").attr("data-title", b);
            d.find(".player-menu-plus").attr("data-sid", 0);
            d.find(".player-menu-plus").attr("data-url", "#plus");
            d.find(".player-menu-plus").attr("data-time", 0);
            d.find(".player-menu-plus").removeClass("in_playlist");
            d.find(".player-menu-plus").unbind("click");
            d.find(".player-menu-plus").click(function () {
                showTopNotify("<strong>\u041d\u0435\u043b\u044c\u0437\u044f \u043f\u0440\u0438 \u043f\u0440\u043e\u0441\u043b\u0443\u0448\u0438\u0432\u0430\u043d\u0438\u0438 \u0440\u0430\u0434\u0438\u043e</strong>");
            ...
            x = -1;
            d.find(".player-time-total, .player-time-loaded, .player-time-current, .player-song-name, .player-time-start, .player-time-end").unbind("click");
            d.find(".player-time-total, .player-time-loaded, .player-time-current, .player-song-name, .player-time-start, .player-time-end").css("cursor", "default");
            d.find(".player-nav-prev").unbind("click");
            d.find(".player-nav-prev").click(function () {
                showTopNotify("<strong>\u041d\u0435\u043b\u044c\u0437\u044f \u043f\u0440\u0438 \u043f\u0440\u043e\u0441\u043b\u0443\u0448\u0438\u0432\u0430\u043d\u0438\u0438 \u0440\u0430\u0434\u0438\u043e</strong>")
            });
            ...
            setTimeout(function () {
                a.post("/ajax/inc/radio", {url: c})
            }, 5E3);
            h = window.soundManager.createSound({
                id: "zv_radio" + Math.random(),
                url: c,
                autoPlay: !0,
                autoLoad: v.autoLoad,
                volume: w,
        ...
        function M() {
            if (1 > this.bytesLoaded / this.bytesTotal) {
                ...
                d.find(".player-time-loaded").css("width", a + "px")
            } else d.find(".player-time-loaded").css("width", "0")
        }
        ...
        function P() {
            ...
            var c = [];
            var g = 0;
            c[g] = Math.floor(a / 3600 % 24);
            0 < c[g] && g++;
            c[g] = Math.floor(a / 60 % 60);
            g++;
            c[g] = Math.floor(a % 60);
            a = !0;
            var e = -1;
            for (g = 0; g < c.length; g++) 10 > c[g] && (1 == g || 2 == g) && (c[g] = "0" + c[g]), "00" == c[g] && g < c.length - 2 && !a ? e = g : a = !0;
            c.splice(0, e + 1);
            c = c.join(":");
            C != c && (d.find(".player-time-start").text(c), d.find(".player-time-end").text(""));
            C = c
        }
        ...
        function I() {
            var b;
            if (v.playNextWhenFinished) if (0 == a(".player-menu-reload").hasClass("active") && 1 < e.length) a.zvPlayer.next(); else if (1 == a(".player-menu-reload").hasClass("active")) h.playState = !1, a.zvPlayer.play(); else {
                if (a.zvPlayer.pause(), d.find(".player-time-start").text(""), d.find(".player-time-end").text(""), h.setPosition(0), y(0), b = a("div[data-play=" + e[f].id + "]").attr("data-next")) if (b = a("div[data-play=" + b + "] .song-play")) {
                    e = [];
                    var c = b.attr("data-url");
                    "#" == c && (c = b.attr("data-href"));
                    t({
            ...
            } else if (a.zvPlayer.pause(), d.find(".player-time-start").text(""), d.find(".player-time-end").text(""), h.setPosition(0), y(0), b = a("div[data-play=" + e[f].id + "]").attr("data-next")) if (b = a("div[data-play=" + b + "] .song-play")) e = [], c = b.attr("data-url"), "#" == c && (c = b.attr("data-href")), t({
                duration: b.attr("data-time"), stream_url: c, title: b.attr("data-title"), id: b.attr("data-sid")
            }), f = -1, a.zvPlayer.addTrack(c, b.attr("data-title"), b.attr("data-sid"), !0, b.attr("data-time"))
        }

        function y(a) {
            a *= k();
            d.find(".player-time-current").css("width", a)
        }

        function J(a, c) {
            var b = F(a / 1E3);
            C != b && (d.find(".player-time-start").text(b), d.find(".player-time-end").text("-" + F(c / 1E3 - a / 1E3)), y(a / c));
            C = b
        }
        ...
        function F(a) {
            ...
            b[1] = Math.floor(a % 60);
            a = !0;
            for (var d = -1, e = 0; e < b.length; e++) 10 > b[e] && 1 == e && (b[e] = "0" + b[e]), "00" == b[e] && e < b.length - 2 && !a ? d = e : a = !0;
            b.splice(0, d + 1);
            return b.join(":")
        }

        var v = a.extend({}, a.fn.zvPlayer.defaults, l), d, r, h, C, B = -1, f = -1, w = 100, n = !0, A, x, E = !0,
            e = [];
        a.zvPlayer.download = function (b, c) {
            c = a(b);
            !isNaN(parseFloat(b)) && isFinite(b) && (c = a(".player-menu-download"));
            var d = 0;
            d = !isNaN(parseFloat(b)) && isFinite(b) ? b : c.attr("data-sid");
            a.getJSON("/ajax/inc/" + d, {}, function (b) {
                var c = (navigator.userAgent || navigator.vendor || window.opera).toLowerCase();
                /ip(ad|hone|od)/.test(c) || -1 === c.indexOf("android") && (/avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|playbook|silk|iemobile|iris|kindle|lge |maemo|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|symbian|treo|up\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino/i.test(c) || /1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|ac(er|oo|s\-)|ai(ko|rn)|al(av|ca|co)|amoi|an(ex|ny|yw)|aptu|ar(ch|go)|as(te|us)|attw|au(di|\-m|r |s )|avan|be(ck|ll|nq)|bi(lb|rd)|bl(ac|az)|br(e|v)w|bumb|bw\-(n|u)|c55\/|capi|ccwa|cdm\-|cell|chtm|cldc|cmd\-|co(mp|nd)|craw|da(it|ll|ng)|dbte|dc\-s|devi|dica|dmob|do(c|p)o|ds(12|\-d)|el(49|ai)|em(l2|ul)|er(ic|k0)|esl8|ez([4-7]0|os|wa|ze)|fetc|fly(\-|_)|g1 u|g560|gene|gf\-5|g\-mo|go(\.w|od)|gr(ad|un)|haie|hcit|hd\-(m|p|t)|hei\-|hi(pt|ta)|hp( i|ip)|hs\-c|ht(c(\-| |_|a|g|p|s|t)|tp)|hu(aw|tc)|i\-(20|go|ma)|i230|iac( |\-|\/)|ibro|idea|ig01|ikom|im1k|inno|ipaq|iris|ja(t|v)a|jbro|jemu|jigs|kddi|keji|kgt( |\/)|klon|kpt |kwc\-|kyo(c|k)|le(no|xi)|lg( g|\/(k|l|u)|50|54|e\-|e\/|\-[a-w])|libw|lynx|m1\-w|m3ga|m50\/|ma(te|ui|xo)|mc(01|21|ca)|m\-cr|me(di|rc|ri)|mi(o8|oa|ts)|mmef|mo(01|02|bi|de|do|t(\-| |o|v)|zz)|mt(50|p1|v )|mwbp|mywa|n10[0-2]|n20[2-3]|n30(0|2)|n50(0|2|5)|n7(0(0|1)|10)|ne((c|m)\-|on|tf|wf|wg|wt)|nok(6|i)|nzph|o2im|op(ti|wv)|oran|owg1|p800|pan(a|d|t)|pdxg|pg(13|\-([1-8]|c))|phil|pire|pl(ay|uc)|pn\-2|po(ck|rt|se)|prox|psio|pt\-g|qa\-a|qc(07|12|21|32|60|\-[2-7]|i\-)|qtek|r380|r600|raks|rim9|ro(ve|zo)|s55\/|sa(ge|ma|mm|ms|ny|va)|sc(01|h\-|oo|p\-)|sdk\/|se(c(\-|0|1)|47|mc|nd|ri)|sgh\-|shar|sie(\-|m)|sk\-0|sl(45|id)|sm(al|ar|b3|it|t5)|so(ft|ny)|sp(01|h\-|v\-|v )|sy(01|mb)|t2(18|50)|t6(00|10|18)|ta(gt|lk)|tcl\-|tdg\-|tel(i|m)|tim\-|t\-mo|to(pl|sh)|ts(70|m\-|m3|m5)|tx\-9|up(\.b|g1|si)|utst|v400|v750|veri|vi(rg|te)|vk(40|5[0-3]|\-v)|vm40|voda|vulc|vx(52|53|60|61|70|80|81|83|85|98)|w3c(\-| )|webc|whit|wi(g |nc|nw)|wmlb|wonu|x700|xda(\-|2|g)|yas\-|your|zeto|zte\-/i.test(c.substr(0, 4)));
                $iframe = a("<iframe>").attr("style", "text-indent:-9999px;border:none;width:0px;height:0px;visibility:hidden;").attr("src", b.url).appendTo("body");
                saveSongIdCookie(d)
            });
            a('[data-sid="' + d + '"].download').addClass("visited").attr("data-original-title", "");
            return !1
        };
        ...
        a.zvPlayer.volume = function (b, c) {
            c || (c = 0);
            0 > b && (b = 0);
            1 < b && (b = 1);
            var e = 100 * b;
            ...
            h && h.setVolume(e);
            1 == c ? (d.find("#player-volume-current").animate({width: 100 * b}), d.find("#player-volume-handle").animate({
                left: 100 * b
            })) : (d.find("#player-volume-current").css("width", 100 * b), d.find("#player-volume-handle").css("left", 100 * b));
            d.find("#player-volume-handle").attr("data-original-title", parseInt(100 * b) + "%");
            w = e
        };
        a.zvPlayer.addToPlaylist = function (b, c, d, k) {
            if (!v.playlist) return !1;
            var g = !1;
            a.ajaxSetup({async: !1});
            a.getJSON("/playlist/add/song/" + d, function (b) {
                if ("login" == b.status) window.location = "/login", g = !0; else if ("max_song_count" == b.status) showTopNotify("<strong>\u041c\u0430\u043a\u0441\u0438\u043c\u0430\u043b\u044c\u043d\u043e\u0435 \u0447\u0438\u0441\u043b\u043e \u043a\u043e\u043c\u043f\u043e\u0437\u0438\u0446\u0438\u0439 100</strong>"), g = "max"; else if ("1" == b.status) {
                    var c = b.id, d = a("#playlist-content-" + c).find(".playlist-count");
                    0 === d.length && (d = a("#playlist-content-nill").clone(), d.attr("id", "playlist-content-" + b.id), d.find("#name-playlist-null").attr("id", "name-playlist-" + b.id).attr("place", b.name).val(b.name), d.find(".playlist-del").click(function () {
                        deletePlaylist(b.id, this)
                    }), d.find(".playlist-ok").click(function () {
                        saveNamePlaylist(b.id)
                    }), d.find(".playlist-play").unbind("click"), d.find(".playlist-play").click(function (b) {
                        a("#plpop-playlists").hide();
                        a("#plpop-songs").show();
                        a("#plpop .playlist-play").removeClass("stop");
                        a(this).addClass("stop");
                        var d = a("#pl-song-list");
                        d.html("loading");
                        a.get("/playlist/" + c, function (b) {
                            d.html(b);
                            setTimeout(function () {
                        ...
                        b.preventDefault()
                    }), d.find(".playlist-rename input").focus(function () {
                        a(this).parents(".playlist-wrap:first").addClass("renamed")
                    }), d.find(".playlist-wrap").hover(function () {
                        a(this).hasClass("renamed") || (a(".playlist-wrap").removeClass("renamed"), a(".playlist-rename input").parents(".playlist-wrap:first").removeClass("renamed"))
                    }, function () {
                    }), a("#plki").append(d), d.show(), d = a("#playlist-content-" + c).find(".playlist-count"));
                    animateCount(a(d), 1)
                } else g = !0
            });
            a.ajaxSetup({async: !0});
            ...
            m.addClass("song_" + d);
            var l = explode("\u2014", c);
            m.find(".song-artist a").attr("href", "/song/" + d).find("span").text(l[0].trim());
            m.find(".song-name a").attr("href", "/song/" + d).find("span").text(l[1].trim());
            m.find(".song-time").text(F(k));
            r.find("#pl-song-list").append(m);
            m.show();
            reloadScroll();
            t({duration: k, stream_url: b, title: c, id: d});
            m.find(".song-play").attr("data-sid", d);
            m.find(".song-play").attr("data-title", c);
            ...
            m.find(".song-delete").click(function () {
                ...
                reloadScroll();
                c == f ? (f--, c = c == e.length ? 0 : c, u(c, n ? !1 : !0)) : c < f && f--
            });
            return g
        };
        a.zvPlayer.addTrack = function (b, c, d, e, f) {
            void 0 === c && (c = "");
            void 0 === e && (e = !1);
            E = e;
            b = t({duration: f, stream_url: b, title: c, id: d});
            e && (x != d ? (a("[data-sid]").removeClass("stop"), a("div[data-play]").removeClass("played"), u(b, !0), a("[data-sid=" + d + "].song-play").addClass("stop"), a("div[data-play=" + d + "].").addClass("played")) : a.zvPlayer.toggle())
        };
        a.zvPlayer.playFirst = function () {
            0 < e.length && (a("[data-sid]").removeClass("stop"), a("div[data-play]").removeClass("played"), u(0, !0), a("[data-sid=" + e[0].id + "].song-play").addClass("stop"), a("div[data-play=" + e[0].id + "].").addClass("played"))
        };
            ...
            r.find("#pl-song-list").children("div").each(function (b, c) {
                ...
                a(c).find(".song-play").click(function () {
                    if (x != a(this).attr("data-sid")) {
                        ...
                        e = [];
                        f = -1;
                        r.find("#pl-song-list").children("div").each(function (b, d) {
...
function explode(a, l) {
    ...
    var m = c.event.special.mousewheel = {
        version: "3.1.12", setup: function () {
            ...
            if (this.removeEventListener) for (var a = k.length; a;) this.removeEventListener(k[--a], l, !1); else this.onmousewheel = null;
...
(function (a) {
    ...
    a.event.special.mwheelIntent = {
        setup: function () {
            ...
            if (this === b.elem || Math.abs(b.pos[0] - f[0]) > c || Math.abs(b.pos[1] - f[1]) > c) return b.elem = this, b.pos = f, c = 250, clearTimeout(l), l = setTimeout(function () {
                c = 10
...
$(document).ready(function () {
    ...
    $(".player-menu-playlists").click(function () {
        "0" == isGuest ? ($(this).toggleClass("active"), "hidden" != $("#plpop").css("visibility") ? ($("#plpop").css("visibility", "hidden"), $.cookie("playListIsOpen", "0")) : ($("#plpop").css("visibility", "visible"), $.cookie("playListIsOpen", "1"))) : window.location = "/login"
    });
...
function addNewPlaylist() {
    $.getJSON("/playlist/create", function (a) {
        if ("max_pl_count" == a.status) showTopNotify("<b>\u041c\u0430\u043a\u0441\u0438\u043c\u0430\u043b\u044c\u043d\u043e\u0435 \u0447\u0438\u0441\u043b\u043e \u043f\u043b\u0435\u0439\u043b\u0438\u0441\u0442\u043e\u0432 20</b>"), out = !0; else if ("login" == a.status) window.location = "/login", out = !0; else if ("" != a.name) {
            ...
            b.find("#name-playlist-null").attr("id", "name-playlist-" + a.id).attr("place", a.name).val(a.name);
            ...
            b.find(".playlist-wrap").hover(function () {
                $(this).hasClass("renamed") || ($(".playlist-wrap").removeClass("renamed"), $(".playlist-rename input").parents(".playlist-wrap:first").removeClass("renamed"))
            }, function () {
            });
            $("#plki").append(b);
            b.show();
    ...
    g.initHtml4 = function () {
        ...
        g.initHtml4.initialized = !0, g.enabled = !0, g.savedHashes = [], g.isLastHash = function (a) {
            ...
            if (f !== !1 && g.busy()) return g.pushQueue({
                scope: g, callback: g.pushState, args: arguments, queue: f
            }), !1;
            g.busy(!0);
            ...
            if (d !== !1 && g.busy()) return g.pushQueue({
                scope: g, callback: g.replaceState, args: arguments, queue: d
            }), !1;
            g.busy(!0);
    ...
    m.init = function () {
        ...
        m.initCore.initialized = !0, m.options = m.options || {}, m.options.hashChangeInterval = m.options.hashChangeInterval || 100, m.options.safariPollInterval = m.options.safariPollInterval || 500, m.options.doubleCheckInterval = m.options.doubleCheckInterval || 500, m.options.storeInterval = m.options.storeInterval || 1E3, m.options.busyDelay = m.options.busyDelay || 250, m.options.debug = m.options.debug || !1, m.options.initialTitle = m.options.initialTitle || d.title, m.intervalList = [], m.clearAllIntervals = function () {
            ...
            return b !== !1 && m.busy() ? (m.pushQueue({
                scope: m, callback: m.setHash, args: arguments, queue: b
            ...
            return a !== !1 && m.busy() ? (m.pushQueue({
                scope: m, callback: m.back, args: arguments, queue: a
        ...
        if (m.emulated.pushState) {
            ...
            if (e !== !1 && m.busy()) return m.pushQueue({
                scope: m, callback: m.pushState, args: arguments, queue: e
            }), !1;
            m.busy(!0);
            ...
            if (m.getHashByUrl(d) && m.emulated.pushState) throw new Error("History.js does not support states with fragement-identifiers (hashes/anchors).");
            if (e !== !1 && m.busy()) return m.pushQueue({
                scope: m, callback: m.replaceState, args: arguments, queue: e
            }), !1;
            m.busy(!0);
            var f = m.createStateObject(b, c, d);
    ...
    $.fn.jScrollPane = function (settings) {
        function JScrollPane(elem, s) {
            ...
            function initialise(s) {
                ...
                if (pane === undefined) {
                    ...
                    container = $('<div class="jspContainer" />').css({
                        "width": paneWidth + "px", "height": paneHeight + "px"
            ...
            function sizeVerticalScrollbar() {
                ...
                try {
                    if (verticalBar.position().left === 0) pane.css("margin-left", scrollbarWidth + "px")
                } catch (err) {
                }
            }
            ...
            function resizeScrollbars() {
                ...
                if (isScrollableH) {
                    ...
                    if (horizontalDragWidth > settings.horizontalDragMaxWidth) horizontalDragWidth = settings.horizontalDragMaxWidth; else if (horizontalDragWidth < settings.horizontalDragMinWidth) horizontalDragWidth = settings.horizontalDragMinWidth;
                ...
                if (isScrollableV) {
                    ...
                    if (verticalDragHeight > settings.verticalDragMaxHeight) verticalDragHeight = settings.verticalDragMaxHeight; else if (verticalDragHeight < settings.verticalDragMinHeight) verticalDragHeight = settings.verticalDragMinHeight;
            ...
            function positionDragY(destY, animate) {
                ...
                if (animate) jsp.animate(verticalDrag, "top", destY, _positionDragY, function () {
            ...
            function initKeyboardNav() {
                ...
                if (settings.hideFocus) {
                    ...
                    elem.css("outline", "");
                    if ("hideFocus" in container[0]) elem.attr("hideFocus", false)
                }
            ...
            function hijackInternalLinks() {
                ...
                $(document.body).delegate('a[href*="#"]', "click", function (event) {
                    ...
                    if (location.href.indexOf("#") !== -1) locationHref = location.href.substr(0, location.href.indexOf("#"));
...
(function (b) {
    ...
    function t(a, c, f) {
        ...
        1 < a.which || (a.metaKey || a.ctrlKey || a.shiftKey || a.altKey) || (location.protocol !== c.protocol || location.host !== c.host || c.hash && c.href.replace(c.hash, "") === location.href.replace(location.hash, "") || c.href === location.href + "#") || (c = {
            url: c.href, container: b(c).attr("data-pjax"), target: c, fragment: null
        ...
        if ((a = a.state) && a.container) {
            ...
            if (c.length) {
                ...
                if (e.state) {
                    ...
                    f ? (c.trigger("pjax:start", [null, d]), a.title && (document.title = a.title), c.html(f), e.state = a, c.trigger("pjax:end", [null, d])) : e(d)
                } else e.state = a
    ...
    function E(a) {
        ...
        "GET" !== e && "POST" !== e && d.append(b("<input>", {
            type: "hidden", name: "_method", value: e.toLowerCase()
        }));
        a = a.data;
    ...
    function v(a, c, e) {
        ...
        e.fragment ? (a = "body" === e.fragment ? g : g.filter(e.fragment).add(g.find(e.fragment)).first(), a.length && (d.contents = a.contents(), d.title || (d.title = a.attr("title") || a.data("title")))) : /<html/i.test(a) || (d.contents = g);
    ...
    function y() {
        ...
        b.pjax.reload = D;
        b.pjax.defaults = {
            timeout: 6500, push: !0, replace: !1, type: "GET", dataType: "html", scrollTo: 0, maxCacheLength: 20
        };
...
(function (window, $, undefined) {
    ...
    $.fn.rrssb = function (options) {
        ...
        for (var key in settings) if (settings.hasOwnProperty(key) && settings[key] !== undefined) settings[key] = encodeString(settings[key]);
...
$(document).ready(function () {
    ...
    $("#close_alert_notify").on("click", function () {
        $("#top_error_notify").fadeOut();
        return false
    });
